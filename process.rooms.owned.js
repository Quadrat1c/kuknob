...
OwnedRoomProcess.prototype.run = function () {
  ...
  kuknob.runProcess(this.room.name + '_defense', RoomDefenseProcess, {
    room: this.room,
    priority: PROCESS_PRIORITY_HIGH,
  });

  this.room.generateLinkNetwork();
  kuknob.runProcess(this.room.name + '_links', ManageLinksProcess, {
    interval: 10,
    room: this.room,
  });

  kuknob.runProcess(this.room.name + '_labs', ManageLabsProcess, {
    room: this.room,
  });
  ...
  // Sing a song.
  kuknob.runProcess(this.room.name + '_song', RoomSongsProcess, {
    room: this.room,
