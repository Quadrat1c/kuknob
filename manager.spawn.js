...
StructureSpawn.prototype.createManagedCreep = function (options) {
    ...
    if (result == newName) {
        ...
        kuknob.log('creeps', this.pos.roomName).info('Spawning new creep:', newName);
...
Room.prototype.spawnCreepByPriority = function (activeSpawn) {
    ...
    if (best.role == 'harvester') {
        ...
        kuknob.log('creeps', this.name).error('trying to spawn unknown creep role:', best.role);
    }
...
Room.prototype.addUpgraderSpawnOptions = function () {
    ...
    if (maxUpgraders == 0 && this.controller.ticksToDowngrade < CONTROLLER_DOWNGRADE[this.controller.level] * 0.5) {
        kuknob.log('creeps', this.name).info('trying to spawn upgrader because controller is close to downgrading', this.controller.ticksToDowngrade, '/', CONTROLLER_DOWNGRADE[this.controller.level]);
...
Room.prototype.manageSpawns = function () {
    ...
    for (let spawnID in roomSpawns) {
...

        {
            ...
            for (var i in remoteHarvestTargets) {
                let position = kuknob.roomIntel(remoteHarvestTargets[i].roomName).getControllerPosition();
                if (position) {
            ...
            for (var i in safeRooms) {
                let position = kuknob.roomIntel(safeRooms[i]).getControllerPosition();
                if (position) {
                    reservePositions.push(position);
                }
            }
