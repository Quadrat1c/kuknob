...
TradeProcess.prototype.instaSellResources = function (resourceType, rooms) {
    ...
    if (amount > (room.terminal.store[resourceType] || 0)) {
      if (!room.memory.fillTerminal) {
        ...
        kuknob.log('trade', bestRoom).info('Preparing', amount, resourceType, 'for selling to', bestOrder.roomName, 'at', bestOrder.price, 'credits each, costing', transactionCost, 'energy');
      }
      else {
        kuknob.log('trade', bestRoom).info('Busy, can\'t prepare', amount, resourceType, 'for selling.');
      }
      return;
    }
    ...
    if (transactionCost > room.terminal.store.energy) {
      if (!room.memory.fillTerminal) {
        ...
        kuknob.log('trade', bestRoom).info('Preparing', transactionCost, 'energy for selling', amount, resourceType, 'to', bestOrder.roomName, 'at', bestOrder.price, 'credits each');
      }
      else {
        kuknob.log('trade', bestRoom).info('Busy, can\'t prepare', transactionCost, 'energy for selling', amount, resourceType);
      }
      return;
    }

    kuknob.log('trade', bestRoom).info('Selling', amount, resourceType, 'to', bestOrder.roomName, 'for', bestOrder.price, 'credits each, costing', transactionCost, 'energy');
...
TradeProcess.prototype.instaBuyResources = function (resourceType, rooms) {
  ...
  if (transactionCost > room.terminal.store.energy) {
    if (!room.memory.fillTerminal) {
      room.prepareForTrading(RESOURCE_ENERGY, transactionCost);
      kuknob.log('trade', bestRoom).info('Preparing', transactionCost, 'energy for buying', amount, resourceType, 'from', bestOrder.roomName, 'at', bestOrder.price, 'credits each');
    }
    else {
      kuknob.log('trade', bestRoom).info('Busy, can\'t prepare', transactionCost, 'energy for buying', amount, resourceType);
    }
    return;
  }

  kuknob.log('trade', bestRoom).info('Buying', amount, resourceType, 'from', bestOrder.roomName, 'for', bestOrder.price, 'credits each, costing', transactionCost, 'energy');
...
TradeProcess.prototype.tryBuyResources = function (resourceType, rooms, ignoreOtherRooms) {
  ...
  if (_.filter(Game.market.orders, (order) => {
    ...
    //kuknob.log('trade').debug('Already buying', resourceType);
    return;
  }
  ...
  if (bestBuyOrder && bestSellOrder) {
    kuknob.log('trade', bestRoom).info(resourceType, 'is currently being bought for', bestBuyOrder.price, 'and sold for', bestSellOrder.price, 'credits.');
    ...
    kuknob.log('trade', bestRoom).info(resourceType, 'is currently being bought for', bestBuyOrder.price);
    offerPrice = bestBuyOrder.price * 0.9;
  }
  else {
    // Nobody is buying this resource, try to get NPC price for it.
    kuknob.log('trade', bestRoom).info('Nobody else is currently buying', resourceType);
    offerPrice = npcPrice;
  }

  kuknob.log('trade', bestRoom).debug('Offering to buy for', offerPrice);
...
TradeProcess.prototype.trySellResources = function (resourceType, rooms) {
  ...
  if (_.filter(Game.market.orders, (order) => order.type == ORDER_SELL && order.resourceType == resourceType).length > 0) {
    //kuknob.log('trade').debug('Already selling', resourceType);
    return;
  }
  ...
  if (bestBuyOrder && bestSellOrder) {
    ...
    kuknob.log('trade', bestRoom).info(resourceType, 'is currently being sold for', bestSellOrder.price);
    offerPrice = bestSellOrder.price / 0.9;
  }
  else {
    // Nobody is selling this resource, try to get a greedy price for it.
    kuknob.log('trade', bestRoom).info('Nobody else is currently selling', resourceType);
    offerPrice = npcPrice * 5;
  }

  kuknob.log('trade', bestRoom).debug('Offering to sell for', offerPrice);
...
TradeProcess.prototype.removeOldTrades = function () {
  for (let id in Game.market.orders) {
    ...
    if (age > 100000 || order.remainingAmount < 100) {
      ...
      kuknob.log('trade', order.roomName).debug('Cancelling old trade', order.type + 'ing', order.remainingAmount, order.resourceType, 'for', order.price, 'each after', age, 'ticks.');
