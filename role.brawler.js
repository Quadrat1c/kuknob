    ...
    if (creep.memory.target) {
        ...
        if (creep.pos.roomName == targetPosition.roomName) {
            ...
            if (creep.memory.body.attack) {
                ...
                if (!creep.room.controller || !creep.room.controller.owner || kuknob.relations.isAlly(creep.room.controller.owner.username)) structures = [];
            ...
            if (creep.memory.body.heal) {
                ...
                if (!damaged || damaged.length == 0) {
                    damaged = creep.room.find(FIND_HOSTILE_CREEPS, {
                        filter: (friendly) => ((friendly.id != creep.id) && (friendly.hits < friendly.hitsMax) && kuknob.relations.isAlly(friendly.owner.username))
                    });
                }
    ...
    if (creep.memory.target) {
        ...
        if (creep.pos.roomName != targetPosition.roomName) {
            if (!this.moveToRoom(targetPosition.roomName)) {
                kuknob.log('creeps').debug(this.name, 'can\'t move from', this.pos.roomName, 'to', targetPosition.roomName);
        ...
        if (target) {
            if (creep.memory.body.attack) {
                var result = creep.moveTo(target, {
                    reusePath: 5,
                    ignoreDestructibleStructures: (!creep.room.controller || !creep.room.controller.owner || (!creep.room.controller.my && !kuknob.relations.isAlly(creep.room.controller.owner.username))),
                });
            }
            else {
...
Creep.prototype.performMilitaryAttack = function () {
    ...
    if (creep.memory.order) {
        ...
        else if (target && (!target.my && (!target.owner || !kuknob.relations.isAlly(target.owner.username)))) {
        ...
        if (!attacked) {
            ...
            if (hostile.length > 0) {
                for (let i in hostile) {
                    ...
                    if (hostile[i].owner && kuknob.relations.isAlly(hostile[i].owner.username)) continue;
            ...
            if (!attacked) {
                ...
                if (creep.room.controller && creep.room.controller.owner && kuknob.relations.isAlly(creep.room.controller.owner.username)) hostile = [];
...
Creep.prototype.performMilitaryHeal = function () {
    ...
    if (creep.memory.order) {
        ...
        if (target && (target.my || (target.owner && kuknob.relations.isAlly(target.owner.username)))) {
